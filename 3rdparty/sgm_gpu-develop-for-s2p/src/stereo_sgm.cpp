...
namespace sgm {
	...
	struct CudaStereoSGMResources {
		...
		CudaStereoSGMResources(int width_, int height_, int disparity_size_, int input_depth_bits_, int output_depth_bits_, int src_pitch_, int dst_pitch_, EXECUTE_INOUT inout_type_, bool verbose) {
            ...
            /* La inicializacion de sgm_engine se come la mayor parte del tiempo */
	...
	void StereoSGM::execute(const void* left_pixels, const void* right_pixels, void* dst) {
        ...
        /* Aca se ejecuta el CT y la disparidad, por ahora
         * d_tmp_left_disp son ceros
         * d_tmp_right_disp son ceros
         * d_input_left tiene la img de la izq
         * d_input_right tiene la img de la der
         * src_pitch_ es el width de la img izq ?
         * dst_pitch_ es el width de la img izq ?
         * */
